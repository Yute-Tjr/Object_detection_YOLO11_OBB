...
```bash
.venv/bin/python scripts/start_terminal_web.py
```

启动器会依次检查 `.env`、三个模型权重、Python 环境、前端依赖、CPU/GPU
配置和 PostgreSQL 连接，然后自动执行 `alembic upgrade head`。所有检查通过后，
它会同时启动 API、单个 Worker 和 Vite 前端，并为日志添加 `[API]`、
`[WORKER]`、`[WEB]` 前缀。任一服务异常退出时会关闭其余进程；按一次
`Ctrl+C` 也会完整关闭三个服务。
...
cd web_frontend && npm run dev -- --host 127.0.0.1 --port 5173
```
